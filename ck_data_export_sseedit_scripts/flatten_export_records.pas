unit Export_Flatten_To_CSV;
{
  This script flattens certain repeated subrecords (e.g. "FormIDs")
  into multiple rows for each subrecord. Then it writes the CSV 
  in ANSI encoding (TStringList.SaveToFile), logging each step 
  for debugging.

  Key points:
    - Flattening logic: if NonAggregatedPaths has "FormIDs", 
      we skip "FormIDs" in baseline, then produce a new row 
      for each child inside "FormIDs".
...
begin
 ...
 NonAggregatedPaths.Add('Responses \ Response');
 NonAggregatedPaths.Add('Link To \ TCLT - Response');

  AddMessage('Initialized "Export_Flatten_To_CSV" script. LogLevel=' + IntToStr(LogLevel));
  Result := 0;
end;
...
begin
  ...
  try
    ...
    csvFileName := 'SkyrimRecordsFlattened_' + FormatDateTime('yyyy-mm-dd_HHNN', Now) + '.csv';
