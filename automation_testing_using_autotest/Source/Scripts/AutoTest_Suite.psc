...
function SetTestType(string testType)
  ...
  ; Make sure our JSON files exist for operations
  if !JsonUtil.JsonExists(gJSONRunFile)
    JsonUtil.Save(gJSONRunFile)
  endif
  
  if !JsonUtil.JsonExists(gJSONStatusesFile)
...
function RegisterNewTest(string testName)
  ; Check if we can determine if test already exists
  bool alreadyExists = false
  int index = JsonUtil.StringListFind(gJSONRunFile, "tests_to_run", testName)
  if index != -1
    alreadyExists = true
  endif
  ...
  if !alreadyExists
    int nbrTests = JsonUtil.StringListAdd(gJSONRunFile, "tests_to_run", testName, false)
    if nbrTests % 5000 == 0
...
int function HexToInt(string hexString)
  ...
  while (idx < strLength)
    ...
    elseIf (currentDigit == "9")
      currentValue = 9
    elseIf (currentDigit == "A" || currentDigit == "a")
      currentValue = 10
    elseIf (currentDigit == "B" || currentDigit == "b")
      currentValue = 11
    elseIf (currentDigit == "C" || currentDigit == "c")
      currentValue = 12
    elseIf (currentDigit == "D" || currentDigit == "d")
      currentValue = 13
    elseIf (currentDigit == "E" || currentDigit == "e")
      currentValue = 14
    elseIf (currentDigit == "F" || currentDigit == "f")
      currentValue = 15
    else
      currentValue = 0
    endIf
    
    ; Manual calculation of hex values
    int power = 1
    int j = 0
    while j < (strLength - idx - 1)
      power *= 16
      j += 1
    endWhile
    
    result += currentValue * power
    idx += 1
  endWhile
  return result
endFunction